...
describe('HexUtils', () => {
    ...
    describe('addressToHex()', async () => {
        for (const addr of [
            ...
            it(addr, async () => {
                const { hexUtils } = await loadFixture(fixture);
                await expect(hexUtils.read.addressToHex([addr])).resolves.toStrictEqual(addr.slice(2).toLowerCase());
            });
        }
    });
    describe('bytesToHex()', async () => {
        for (let n = 0; n <= 33; n++) {
            ...
            it(data, async () => {
                const { hexUtils } = await loadFixture(fixture);
                await expect(hexUtils.read.bytesToHex([data])).resolves.toStrictEqual(data.slice(2));
            });
        }
    });
    describe('unpaddedUintToHex()', async () => {
        for (let n = 0; n <= 32; n++) {
            ...
            it(`0x${hex}`, async () => {
                const { hexUtils } = await loadFixture(fixture);
                await expect(hexUtils.read.unpaddedUintToHex([uint, true]), 'true').resolves.toStrictEqual(hex);
                await expect(hexUtils.read.unpaddedUintToHex([uint, false]), 'false').resolves.toStrictEqual(hex.length & 1 ? `0${hex}` : hex);
            });
        }
    });
});
