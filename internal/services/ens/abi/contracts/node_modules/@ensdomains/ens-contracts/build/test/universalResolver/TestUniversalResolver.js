...
import { encodeErrorResult, HttpRequestError, keccak256, namehash, parseAbi, toBytes, toFunctionSelector, toHex, zeroAddress, } from 'viem';
...
async function fixture() {
    ...
    after(bg.shutdown);
    const UniversalResolver = await hre.viem.deployContract('UniversalResolver', [ens.ENSRegistry.address, [bg.localBatchGatewayUrl]], {
        client: {
...
describe('UniversalResolver', () => {
    describe('findResolver()', () => {
        it('unset', async () => {
            ...
            const [resolver, node, offset] = await F.UniversalResolver.read.findResolver([dnsEncodeName(testName)]);
        ...
        it('extended', async () => {
            ...
            expectVar({ resolver }).toEqualAddress(F.Shapeshift1.address);
            expectVar({ node }).toStrictEqual(namehash(testName));
            expectVar({ offset }).toStrictEqual(BigInt(1 + toBytes(testName.split('.')[0]).length));
        });
        it('auto-encrypted', async () => {
            ...
            const [resolver, node, offset] = await F.UniversalResolver.read.findResolver([dnsEncodeName(name)]);
            expectVar({ resolver }).toEqualAddress(F.Shapeshift1.address);
    ...
    describe('resolve()', () => {
        ...
        it('old w/multicall (1 revert)', async () => {
            ...
            const bundle = bundleCalls(makeResolutions({
                name: testName,
                primary: {
                    name: testName,
                },
    ...
    describe('reverse()', () => {
        ...
        it('unset name()', async () => {
            ...
            await F.Shapeshift1.write.setResponse([res.call, res.answer]);
            const [name, resolver, reverseResolver] = await F.UniversalResolver.read.reverse([F.owner, COIN_TYPE_ETH]);
        ...
        it('offchain extended name() + offchain extended addr()', async () => {
            ...
            await F.Shapeshift2.write.setResponse([res.call, res.answer]);
            const [name, resolver, reverseResolver] = await F.UniversalResolver.read.reverse([F.owner, coinType]);
            expectVar({ name }).toStrictEqual(testName);
            expectVar({ resolver }).toEqualAddress(F.Shapeshift2.address);
            expectVar({ reverseResolver }).toEqualAddress(F.Shapeshift1.address);
        });
