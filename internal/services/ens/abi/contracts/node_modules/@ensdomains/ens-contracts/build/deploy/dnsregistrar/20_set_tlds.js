import { encodeFunctionData, namehash, parseAbi, parseEther, } from 'viem';
...
const func = async function (hre) {
    ...
    const suffixList = await (await fetch('https://publicsuffix.org/list/public_suffix_list.dat', {
        headers: {
            Connection: 'close',
        },
    })).text();
    let suffixes = await Promise.all(suffixList
        .split('\n')
        ...
        if (!network.saveDeployments && network.tags.test)
            return returnData;
        const owner = await registry.read.owner([node]);
        if (owner === dnsRegistrar.address) {
            console.log(`Skipping .${suffix}; already owned`);
            return null;
        }
        const isPublicSuffix = await publicSuffixList.read.isPublicSuffix([
            encodedSuffix,
        ]);
        if (!isPublicSuffix) {
    ...
    })).then((suffixes) => suffixes.filter((suffix) => suffix !== null));
    console.log(`Processing ${suffixes.length} public suffixes...`);
    const multicallExistingBytecode = await publicClient.getBytecode({
        address: multicallAddress,
    });
    // devnet deploy, deploy multicall
    if (!multicallExistingBytecode && !network.saveDeployments) {
        const balanceHash1 = await deployer.wallet.sendTransaction({
            to: '0x05f32B3cC3888453ff71B01135B34FF8e41263F2',
            value: parseEther('1'),
        });
        await viem.waitForTransactionSuccess(balanceHash1);
        const balanceHash = await deployer.wallet.sendTransaction({
            to: multicallPreparationAddress,
            value: parseEther('1'),
        });
        await viem.waitForTransactionSuccess(balanceHash);
        ...
        await viem.waitForTransactionSuccess(deployHash);
    }
    const allowUnsafe = network.tags.test && !network.saveDeployments;
    ...
    console.log(`Waiting on ${pendingTransactions.length} transactions to complete...`);
    await Promise.all(pendingTransactions.map(async (hash) => viem.waitForTransactionSuccess(hash)));
};
func.tags = ['settlds'];
func.dependencies = [];
export default func;
