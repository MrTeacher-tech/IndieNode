...
import { decodeFunctionResult, encodeFunctionData, encodeFunctionResult, getAddress, namehash, parseAbi, } from 'viem';
...
export function getParentName(name) {
...
export function bundleCalls(calls) {
    if (calls.length == 1) {
        return {
            call: calls[0].call,
    ...
    return {
        ...
        unbundle: (data) => decodeFunctionResult({
            abi: RESOLVE_MULTICALL,
            data,
        }),
        expect(answer) {
...
export function makeResolutions(p) {
    const v = [];
