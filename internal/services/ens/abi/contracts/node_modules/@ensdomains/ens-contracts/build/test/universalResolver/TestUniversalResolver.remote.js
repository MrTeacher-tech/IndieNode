...
async function fixture() {
    ...
    after(bg.shutdown);
    return hre.viem.deployContract('UniversalResolver', [ENS_REGISTRY, [bg.localBatchGatewayUrl]], {
        client: {
...
;
(isHardhatFork() ? describe : describe.skip)('UniversalResolver @ mainnet', () => {
    describe('resolve()', () => {
    ...
    describe('reverse()', () => {
        for (const x of KNOWN_PRIMARIES) {
            it(`${x.title}: ${shortCoin(x.coinType)} ${x.encodedAddress}`, async () => {
                const F = await loadFixture(fixture);
