...
import { hexToBigInt, labelhash, namehash, stringToHex, zeroAddress, zeroHash, } from 'viem';
...
describe('MigrationHelper', () => {
    it('should allow the owner to set a migration target', async () => {
        ...
        expect(await migrationHelper.read.migrationTarget()).toEqualAddress(ownerAccount.address);
    });
    it('should not allow non-owners to set migration targets', async () => {
        const { migrationHelper, ownerAccount, registrantAccount } = await loadFixture(fixture);
        await expect(migrationHelper)
            .write('setMigrationTarget', [ownerAccount.address], {
    ...
    it('should refuse to migrate unwrapped names to the zero address', async () => {
        const { baseRegistrar, migrationHelper, registrantAccount } = await loadFixture(fixture);
        const ids = [labelhash('test'), labelhash('test2')].map((v) => hexToBigInt(v));
        for (let id of ids) {
        ...
        await baseRegistrar.write.setApprovalForAll([migrationHelper.address, true], { account: registrantAccount });
        await expect(migrationHelper)
            .write('migrateNames', [
            registrantAccount.address,
            ids,
    ...
    it('should migrate unwrapped names', async () => {
        const { baseRegistrar, migrationHelper, ownerAccount, registrantAccount } = await loadFixture(fixture);
        const ids = [labelhash('test'), labelhash('test2')].map((v) => hexToBigInt(v));
        for (let id of ids) {
            await baseRegistrar.write.register([
                id,
    ...
    it('should only allow controllers to migrate unwrapped names', async () => {
        ...
        await baseRegistrar.write.setApprovalForAll([migrationHelper.address, true], { account: registrantAccount });
        await expect(migrationHelper)
            .write('migrateNames', [registrantAccount.address, ids, stringToHex('test')], { account: registrantAccount })
            .toBeRevertedWithString('Controllable: Caller is not a controller');
    });
    it('should migrate wrapped names', async () => {
        const { nameWrapper, migrationHelper, ownerAccount, registrantAccount } = await loadFixture(fixture);
        ...
        await expect(migrationHelper)
            ...
            .withArgs(migrationHelper.address, registrantAccount.address, ownerAccount.address, ids, ids.map(() => 1n));
    });
    it('should refuse to migrate wrapped names to the zero address', async () => {
        const { nameWrapper, migrationHelper, registrantAccount } = await loadFixture(fixture);
        const labels = ['test', 'test2'];
        const ids = labels.map((label) => hexToBigInt(namehash(label + '.eth')));
        for (let label of labels) {
            await nameWrapper.write.registerAndWrapETH2LD([
    ...
    it('should only allow controllers to migrate wrapped names', async () => {
        ...
        await expect(migrationHelper)
            .write('migrateWrappedNames', [registrantAccount.address, ids, stringToHex('test')], { account: registrantAccount })
            .toBeRevertedWithString('Controllable: Caller is not a controller');
    });
});
