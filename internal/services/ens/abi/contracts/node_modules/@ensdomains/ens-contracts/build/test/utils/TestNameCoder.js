...
describe('NameEncoder', () => {
    describe('valid', () => {
        for (let [title, ens] of [
            ...
            it(title, async () => {
                ...
                await expect(F.read.decode([dns]), 'decode').resolves.toStrictEqual(dnsDecodeName(dns));
                let pos = 0;
                while (true) {
                    await expect(F.read.namehash([dns, BigInt(pos)]), `namehash: ${ens}`).resolves.toStrictEqual(namehash(ens));
                    if (!ens)
                        break;
    ...
    describe('decode() failure', () => {
        for (const dns of ['0x', '0x02', '0x0000', '0x1000']) {
            it(dns, async () => {
                const F = await loadFixture(fixture);
                await expect(F)
                    .read('decode', [dns])
