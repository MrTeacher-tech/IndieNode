...
import { chainFromCoinType, COIN_TYPE_ETH, EVM_BIT, getReverseName, shortCoin, } from '../fixtures/ensip19.js';
        ...
        for (const addr of [
            ...
            it(addr, async () => {
                ...
                for (const coinType of coinTypes) {
                    await expect(F.read.reverseName([addr, coinType]), shortCoin(coinType)).resolves.toStrictEqual(getReverseName(addr, coinType));
                }
            });
        }
    });
    describe('chainFromCoinType()', () => {
        for (const coinType of coinTypes) {
            it(shortCoin(coinType), async () => {
                const F = await loadFixture(fixture);
                await expect(F.read.chainFromCoinType([coinType])).resolves.toStrictEqual(chainFromCoinType(coinType));
            });
        }
    });
});
